...
def ContourFaster(pos):
    ...
    for i in range(0,Swimmers.num):
        ...
        f[sliceY,sliceX] = f[sliceY,sliceX] + (1/(4*pi*Std))*exp(-((Swimmers.xm[sliceY,sliceX]-p[0])**2+(Swimmers.ym[sliceY,sliceX]-p[1])**2)/4/Std)
        if ((p[0])**2<8*Std):
            sliceX = slice(lowerXplus,upperXplus)
            sliceY = slice(lowerY,upperY)
            f[sliceY,sliceX] = f[sliceY,sliceX] + (1/(4*pi*Std))*exp(-((Swimmers.xm[sliceY,sliceX]-p[0]-Swimmers.L)**2+(Swimmers.ym[sliceY,sliceX]-p[1])**2)/4/Std)
            A = 1
        if ((p[0]-Swimmers.L)**2<8*Std):
        ...
        if ((p[1])**2<8*Std):
            ...
            f[sliceY,sliceX] = f[sliceY,sliceX] + (1/(4*pi*Std))*exp(-((Swimmers.xm[sliceY,sliceX]-p[0])**2+(Swimmers.ym[sliceY,sliceX]-p[1]-Swimmers.L)**2)/4/Std)
            C = 1
        if ((p[1]-Swimmers.L)**2<8*Std):
            sliceX = slice(lowerX,upperX)
        ...
        if (A == 1 and C == 1): #Plankton in Lower Left Corner
            sliceX = slice(lowerXplus,upperXplus)
            sliceY = slice(lowerYplus,upperYplus)
            f[sliceY,sliceX] = f[sliceY,sliceX] + (1/(4*pi*Std))*exp(-((Swimmers.xm[sliceY,sliceX]-p[0]-Swimmers.L)**2+(Swimmers.ym[sliceY,sliceX]-p[1]-Swimmers.L)**2)/4/Std)
        if (A == 1 and D == 1): #Plankton in Lower Left Corner
            sliceX = slice(lowerXplus,upperXplus)
            sliceY = slice(lowerYminus,upperYminus)
            f[sliceY,sliceX] = f[sliceY,sliceX] + (1/(4*pi*Std))*exp(-((Swimmers.xm[sliceY,sliceX]-p[0]-Swimmers.L)**2+(Swimmers.ym[sliceY,sliceX]-p[1]+Swimmers.L)**2)/4/Std)
...
```{python}
start = time.process_time()
ContoursAll2 = np.zeros((len(pos_store),meshsize,meshsize))
for k in range(0,len(scalar_store)):
    ContoursAll2[k,:,:] = ContourFaster(pos_store[k])
print("Time:", time.process_time()-start)
print("Rel err:",amax(abs(ContoursAll[-1,:,:]-ContoursAll2[-1,:,:]))/amax(abs(ContoursAll[-1,:,:])))
#np.save('Contours', ContoursAll)
```
