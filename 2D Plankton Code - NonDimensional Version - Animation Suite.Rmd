...
Swimmers = PS.Plankton(DP.constantDep,N = meshsize,depMaxStr=.01,Const=3,L=LL,k=0.02,epsilon=1e-3,speed=1,
             lambda0=1,kappa=.2,beta=5,depThreshold=0.05, depTransWidth=0.03, num = numb, c0=0.012)
...
for nn in NUMB:
    ...
    Swimmers2 = PS.Plankton(DP.constantDep,N = meshsize,depMaxStr=.01,Const=3,L=LL,k=0.02,epsilon=1e-3,speed=1,
                 lambda0=1,kappa=.2,beta=5,depThreshold=0.05, depTransWidth=0.03, num = numb, c0=0.012)
    ...
    for l in range(0,lenn):
        for k in range(0,lenn):
            pos2 = np.append(pos2,[np.array([np.mod(k*(Swimmers2.L*1/(lenn+1)) + 0.5*(Swimmers2.L*1/(lenn+1)),Swimmers2.L),
                                            np.mod(l*(Swimmers2.L*1/(lenn+1)) + 0.5*(Swimmers2.L*1/(lenn+1)),Swimmers2.L)])],axis=0)
            th  = rand()*2*pi
    ...
    for plot in range(0,1):
        for k in range(0,simTime):
            Swimmers.UpdateSlice(Swimmers2.scalar,pos2,vel2)
...
plt.figure()
j = 3
plt.plot(pos_store2[j][:,0],pos_store2[j][:,1],'ro')
plt.pcolormesh(Swimmers.xm,Swimmers.ym,ChemStoreSlice[j,:,:]-ChemStoreReg[j,:,:])
plt.colorbar()
plt.title(r'Err between Slicing and Regular Update')
```
